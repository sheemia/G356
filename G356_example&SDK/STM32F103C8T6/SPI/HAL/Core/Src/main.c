...
int main(void)
{
  ...
  while (1)
  {
    ...
    for (int retry = 0; retry < 3; retry++) {
        ...
        HAL_Delay(1); // 每次失败后等待从机复位/预装载完毕再试
    }
...
/**
  ...
  *        HSI(8MHz, internal) / 2 -> PLL x16 -> SYSCLK 64MHz, APB1 32MHz,
  *        APB2 64MHz. Deliberately HSI-based (no external HSE crystal
  *        dependency) so this demo runs unmodified regardless of which HSE
  *        crystal (or none) is fitted on the user's particular F103C8T6
  *        minimum-system-board. F103's HSI/PLL path tops out at 64MHz
  *        (HSI/2 = 4MHz input, x16 max multiplier), short of the 72MHz an
  *        8MHz HSE + x9 PLL would reach, but that's an acceptable tradeoff
  *        for this demo versus depending on a possibly-mismatched HSE_VALUE.
  * @retval None
  */
void SystemClock_Config(void)
{
