...
int main(void)
{
    ...
    USART2_G356_Init(115200);

    printf("\r\nG356 STM32F407 Standard UART example\r\n");

    while (1) {
        if (USART2_ReadByte(&rx_byte)) {
            G356_FrameStatus status = G356_FeedByte(rx_byte, frame);
            if (status == G356_FRAME_VALID && G356_ParseData(frame, &data)) {
