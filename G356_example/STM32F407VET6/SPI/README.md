# G356 AHRS 陀螺仪模块 SPI 通信接收示例工程 (STM32F407VET6, HAL 库)
...
本工程是针对 **G356 六轴姿态传感器模块** 的 SPI 通信示例代码，运行在 **STM32F407VET6 最小系统板** 上，使用 **STM32Cube HAL 库**。本工程作为 **SPI 主机 (Controller)** 运行，通过 SPI1 接口读取作为 **SPI 从机 (Peripheral)** 的 G356 模块的姿态及惯性传感器数据，并通过另一路串口 (USART1) 将解析后的数据以 115200 波特率打印输出，便于用户查看和二次开发。

可直接复用到你自己项目里的驱动代码是 `Core/Inc/g356_driver.h` / `Core/Src/g356_driver.c`（见第 4 节），不需要连带这个 demo 的主循环一起拷贝。

本工程没有用 STM32CubeMX 重新生成校验（本机没有装 CubeMX），是直接在已有的 `SPI.ioc` 骨架上手写补全 SPI1/USART1/GPIO 初始化代码，但已经用 Keil `UV4.exe -b` 离线编译验证通过（0 Error, 0 Warning）。`SPI.ioc` 已同步更新为对应配置，方便你之后用 CubeMX 重新打开/重新生成。
...
| **GND** | **GND** | 电源地 | 必须共地 |
| **CS (片选)** | **PA4** | SPI 片选信号 (GPIO 软件输出) | 主机拉低启动通信，拉高结束；NSS 软件管理，不占用硬件 NSS 引脚 |
| **SCLK (时钟)** | **PA5 (SPI1_SCK)** | SPI 时钟信号 | 时钟频率 ~2.625MHz (APB2=84MHz/32) |
| **MOSI (主机输出)** | **PA7 (SPI1_MOSI)** | SPI 主机输出从机输入 | 物理接线可悬空不接，G356 仅发送数据 |
| **MISO (主机输入)** | **PA6 (SPI1_MISO)** | SPI 主机输入从机输出 | 从机返回数据帧 |
...
> SPI 读取约束：主机每次应在一次 CS 低电平期间连续读取完整 56 字节，读完后再释放 CS。不要把同一帧拆成多次 CS 读取；模块内部会按一次 CS 低电平事务锁存一帧快照。

## 2. 系统时钟与 SPI 通信配置

* **系统时钟**：内部 HSI 16MHz 振荡器 -> PLL (M=16, N=336, P=2, Q=7) -> SYSCLK 168MHz，AHB 168MHz，APB1 42MHz，APB2 84MHz（`SystemClock_Config()`，`Core/Src/main.c`，未启用 Overdrive，168MHz 在 Scale1 下无需开启）。**故意不用外部 HSE 晶振**：不同厂家/批次的 F407VET6 最小系统板实际焊的 HSE 晶振频率并不统一（常见 8MHz，也有 25MHz 等），而 `stm32f4xx_hal_conf.h` 里的 `HSE_VALUE` 默认是 CubeMX 模板值 25MHz——如果板子实际晶振频率与这个宏不一致，HAL 库算出来的 USART 波特率分频值、`HAL_Delay()` 节拍都会跟着算错（且不会报错，只是悄悄变成乱码/计时不准）。HSI 是芯片内部出厂校准的振荡器，`HSI_VALUE` 固定为正确的 16MHz，不存在这个隐患，因此本 demo 不依赖、也不要求外部晶振即可正常运行。
* **SPI 模式**：Mode 0 (CPOL=0, CPHA=0)，`SPI_MODE_MASTER`，8 位字长，MSB 先传，NSS 软件管理（CS 由 GPIO PA4 手动控制，见 `Core/Src/spi.c`）。
* **通信时钟**：APB2(84MHz) / 32 ≈ **2.625MHz**，与 MSPM0G3507 参考实现的 2MHz 接近，可作为起点；G356 是从机，时钟完全由主机决定，理论上可以调快，但本示例没有在真实硬件上验证过更高速率。
...
* **`Core/Inc/g356_driver.h` / `Core/Src/g356_driver.c`** —— 可直接整体拷贝复用的 G356 驱动模块：
    1. **全双工突发读取** (`G356_ReadPacket`)：拉低片选 (PA4)，用 `HAL_SPI_TransmitReceive()` 一次性读取 56 字节（CS 全程保持低电平，避免帧内字节被中途撕裂），读取完成后拉高片选，过滤不合法的帧头、帧尾、和校验。
    2. **数据安全解析** (`G356_ParseData`)：用 `memcpy` 安全提取 `float` 数据（Cortex-M4 本身支持非对齐访问，这里仍用 memcpy 保持跨平台一致性）；换算用乘以倒数代替浮点除法。
    * 依赖 `Core/Inc/spi.h` 的 `hspi1` 句柄和 `Core/Inc/main.h` 的 `GPIO_CS_*` 宏，复用到自己项目时按需调整这些名字。
* **`Core/Src/spi.c`** —— `MX_SPI1_Init()` + `HAL_SPI_MspInit()`，CubeMX 风格的 SPI1 外设初始化。
* **`Core/Src/usart.c`** —— `MX_USART1_UART_Init()` + `HAL_UART_MspInit()`，调试串口初始化。
* **`Core/Src/gpio.c`** —— CS 引脚 (PA4) 的 GPIO 初始化。
...
1. **导入工程**：打开 Keil MDK-ARM (uVision)，直接打开 `MDK-ARM/SPI.uvprojx`。
...
4. **打开串口调试工具**：在 PC 端打开任意串口助手（波特率 `115200`，数据位 `8`，停止位 `1`，无校验），即可看到实时输出的姿态数据与惯性原始数据：
    ...
    =========================================
       ...
       Communication: SPI1 (Mode 0, ~2.6MHz) 
    =========================================
