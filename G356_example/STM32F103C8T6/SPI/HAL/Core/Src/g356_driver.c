...
{
    uint8_t tx_dummy[G356_FRAME_SIZE];
    ...
    // 3. 全双工突发读取 56 字节 (一次 HAL 调用完成，CS 全程保持低电平)
    HAL_StatusTypeDef status = HAL_SPI_TransmitReceive(&hspi2, tx_dummy, frame_buf, G356_FRAME_SIZE, 100);
