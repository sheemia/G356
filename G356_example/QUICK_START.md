...
3. UART 项目中，推荐用 RX 中断或 DMA 持续接收，并把每个字节喂给 `G356_FeedByte()`。
...
`G356_Update()` 也可以用于快速验证或从软件 ring buffer 轮询取字节，但不建议在忙碌应用中低频直接轮询 MCU 硬件 UART FIFO。

## 3. UART FIFO 溢出风险

G356 UART 数据是连续输出流。实时控制项目中不要依赖低频主循环轮询 MCU UART FIFO。如果主循环里有显示、printf、PID、无线通信等耗时任务，可能导致 UART FIFO overrun，表现为校验失败、丢帧、姿态跳变。

推荐做法：

- 快速确认模块是否有数据：使用 PC 工具或简单阻塞 demo。
- 集成到真实产品：使用 UART RX 中断或 DMA。
- 打印调试信息：低频打印，例如每 20 帧或 50 帧打印一次。
- 不要在 UART 接收中断里做大量 printf。

## 4. 需要完整例程时，再选择 MCU 示例工程
